...
def upgrade() -> None:
    ...
    json_path = os.path.join(
        current_dir, "..", "..", "seeds", "india_states_districts.json"
    )
    ...
    if count == 0:
        ...
        for item in data:
            ...
            result = conn.execute(
                sa.text(
                    "INSERT INTO states (name, code) VALUES (:name, :code) RETURNING id"
                ),
            ...
            for dist_name in districts:
                conn.execute(
                    sa.text(
                        "INSERT INTO districts (state_id, name) VALUES (:state_id, :name)"
                    ),
