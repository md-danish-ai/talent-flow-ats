    ...
    def generate_image(self, image_prompt: str) -> Optional[str]:
        ...
        try:
            ...
            image_data = self.client.text_to_image(
                image_prompt, model="black-forest-labs/FLUX.1-schnell"
            )
...
"""
        ...
        if "choice" in question_type.lower() or "mcq" in question_type.lower():
            ...
            constraints.append(
                "- Provide 'correct_answer' as a single key (e.g., 'B')."
            )

        if "passage" in question_type.lower():
            constraints.append(
                "- Include a detailed, professional reading passage for each question in the 'passage' field."
            )

        if "image" in question_type.lower():
            constraints.append(
                "- This is an IMAGE-based question. You MUST provide a detailed 'image_prompt' field describing what the image should show (e.g., 'A professional diagram of a marketing funnel')."
            )

        if "typing" in question_type.lower():
            constraints.append(
                "- This is a TYPING TEST. You MUST provide the full paragraph to be typed in the 'passage' field (200-300 words)."
            )
            constraints.append(
                "- Set 'question_text' to 'Type the following paragraph as accurately and quickly as possible.'"
            )
            constraints.append("- Leave 'options' as an empty object {}.")

        if "lead" in question_type.lower() or "contact" in question_type.lower():
            constraints.append(
                "- This is a DATA COLLECTION task. Set 'question_text' to a professional company bio, a scenario, or a simulated URL (e.g., 'Extract details from: https://acme-industries.com/about')."
            )
            constraints.append("- Leave 'options' as an empty object {}.")
