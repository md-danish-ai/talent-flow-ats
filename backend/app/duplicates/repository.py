...
def get_notifications(
    db: Session, pagination: PaginationParams, is_read: Optional[bool] = None
...
def get_counts(db: Session):
    unread = (
        db.query(AdminNotification).filter(AdminNotification.is_read.is_(False)).count()
    )
    read = (
        db.query(AdminNotification).filter(AdminNotification.is_read.is_(True)).count()
    )
    return unread, read
...
def update_notification_status(db: Session, ids: List[int], is_read: bool):
    notifications = (
        db.query(AdminNotification).filter(AdminNotification.id.in_(ids)).all()
    )
