...
async def assign_interviews(
    ...
    try:
        ...
        existing = (
            db.query(repository.InterviewEvaluation)
            .filter(
                repository.InterviewEvaluation.user_id == payload.user_id,
                repository.InterviewEvaluation.project_lead_id
...
def get_lead_tasks(
    ...
    try:
        tasks, total = repository.get_evaluations_by_lead(
            db, lead_id, status, page, limit
        )
        return ResponseHandler.success(
            data=tasks,
            pagination={
...
async def submit_evaluation(
    ...
    try:
        ...
        return ResponseHandler.success(
            data=evaluation, message="Evaluation submitted successfully."
        )
    except HTTPException as e:
        raise e
    except Exception as e:
