...
class DashboardService:
    async def get_overview(
        ...
        try:
            ...
            grade_query = (
                db.query(InterviewRecord.overall_grade, func.count(InterviewRecord.id))
                .filter(
                    func.date(InterviewRecord.submitted_at) >= filter_start,
                    func.date(InterviewRecord.submitted_at) <= filter_end,
                )
                .group_by(InterviewRecord.overall_grade)
                .all()
            )
            ...
            return DashboardOverviewResponse(stats=stats, today_pulse=today_pulse)
