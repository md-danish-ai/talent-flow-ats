...
def insert_users():
    ...
    try:
        ...
        cur.execute(
            "SELECT id FROM classifications WHERE type = 'exam_level' AND code = 'FRESHER' LIMIT 1"
        )
        fresher_level = cur.fetchone()
        fresher_level_id = fresher_level[0] if fresher_level else 9

        print(
            f"Using Department ID: {kpo_dept_id} (KPO), Level ID: {fresher_level_id} (FRESHER)"
        )
        ...
        for i, user in enumerate(users):
            day_offset = i // 10
            current_date = (base_date + timedelta(days=day_offset)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
