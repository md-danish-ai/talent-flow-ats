 ...
 */
export const isExclusiveClassification = (classificationOrName: Classification | string): boolean => {
  if (!classificationOrName) return false;
  
  if (typeof classificationOrName === "string") {
    const n = normalize(classificationOrName);
    return SPECIAL_CATEGORIES.some(c => n.includes(c)) || SPECIAL_QUESTION_TYPE_CODES.some(c => n.includes(c));
  }
  ...
  return SPECIAL_CATEGORIES.some(c => nName.includes(c)) || 
...
export const filterSubjectsForQuestionType = (
  ...
  const activeQT = typeof currentQuestionType === "string" 
      ? questionTypesPool?.find(t => t.code === currentQuestionType) || currentQuestionType
      : currentQuestionType;

  if (isExclusiveClassification(activeQT)) {
    const qtName = typeof activeQT === "string" ? activeQT : (activeQT.name || activeQT.code);
    const nQT = normalize(qtName);
    
    return subjects.filter((subject) => {
      const nS = normalize(subject.name || subject.code);
      return isExclusiveClassification(subject) && 
        (nS === nQT || 
