...
export const GradeBadge = ({ gradeLabel, value, className }: GradeBadgeProps) => {
  ...
  return (
    <div
      className={`flex items-center justify-between px-4 py-2.5 rounded-sm border-2 ${style} shadow-sm transition-all duration-300 ${className}`}
    >
      <span className="font-black text-xs uppercase tracking-widest leading-none">
        {gradeLabel}
      </span>
      <div className="w-1 h-3 rounded-sm bg-current opacity-20 mx-3 shrink-0" />
