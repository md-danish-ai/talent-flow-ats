...
import { updateBasicInfo } from "@lib/api/auth";
import {
  createAdminSchema,
...
interface EditProjectLeadFormProps {
  lead: UserListResponse;
  onSuccess: () => void;
  onCancel: () => void;
}

export const EditProjectLeadForm = ({
  lead,
  onSuccess,
  onCancel,
}: EditProjectLeadFormProps) => {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        await updateBasicInfo(lead.id, value);
  ...
  return (
    ...
    >
      {errorDetails && (
        <div className="bg-red-50 dark:bg-red-950/40 text-red-500 p-3 rounded-md text-sm mb-4 border border-red-200 dark:border-red-900/50">
          {errorDetails}
        </div>
      )}
