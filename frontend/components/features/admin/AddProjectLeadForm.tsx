...
import { createProjectLead } from "@lib/api/auth";
import {
  createAdminSchema,
...
interface AddProjectLeadFormProps {
  onSuccess: () => void;
  onCancel: () => void;
}

export const AddProjectLeadForm = ({
  onSuccess,
  onCancel,
}: AddProjectLeadFormProps) => {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        await createProjectLead(value);
        onSuccess();
      } catch (error: unknown) {
        console.error("Failed to create project lead:", error);
        setErrorDetails(
