        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[2.5rem] p-10 shadow-xl shadow-slate-200/50 dark:shadow-black/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="flex items-center gap-8">
            ...
            <div className="space-y-1">
              <Typography variant="h2" weight="black" className="capitalize tracking-tight">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-zinc-800/50">
                <div className="space-y-2">
                  <Typography variant="body5" weight="black" className="text-slate-400 uppercase tracking-tighter text-[10px]">
                    Present Address
                  </Typography>
                  <p className="text-sm font-medium leading-relaxed text-slate-700 dark:text-zinc-300">
                    {personalDetails?.presentAddressLine1}, {personalDetails?.presentCity}, {personalDetails?.presentDistrict}, {personalDetails?.presentState} — {personalDetails?.presentPincode}
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            <div className="space-y-6">
              ...
              <div className="flex gap-4 pt-4 border-t border-slate-100 dark:border-zinc-800/50">
                ...
                <DetailItem
                  label="Exp. Salary"
                  value={otherDetails?.expectedSalary ? `₹${otherDetails.expectedSalary}` : "—"}
                />
              </div>
            </div>
          </SectionCard>
        ...
        <div className="md:col-span-2">
          ...
          >
            <div className="space-y-8">
              {!workExperienceDetails?.length || workExperienceDetails[0].company === "" ? (
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                        <div className="flex flex-col">
                          <Typography variant="body1" weight="black" className="tracking-tight uppercase">
                            {work.company}
                          </Typography>
                          <Typography variant="body3" weight="bold" className="text-blue-600 dark:text-blue-400">
                            {work.designation}
                          </Typography>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                        <div className="p-4 rounded-2xl bg-slate-50/50 dark:bg-zinc-950/20 border border-slate-100 dark:border-zinc-800/50">
                          <Typography variant="body5" weight="black" className="text-slate-400 uppercase tracking-tighter text-[9px] mb-1 block">
                            Departure Reason
                          </Typography>
                          <Typography variant="body4" className="text-slate-600 dark:text-zinc-400 italic">
                            &quot;{work.reason || "N/A"}&quot;
                          </Typography>
                        </div>
                        <div className="p-4 rounded-2xl bg-blue-50/20 dark:bg-blue-900/10 border border-blue-100/50 dark:border-blue-900/20">
                          <Typography variant="body5" weight="black" className="text-blue-400 uppercase tracking-tighter text-[9px] mb-1 block">
                            Last Package
                          </Typography>
                          <Typography variant="body3" weight="black" className="text-blue-700 dark:text-blue-300">
                            ₹{work.salary}
                          </Typography>
                        </div>
                      </div>
              ...
              <table className="w-full text-sm border-collapse min-w-[600px]">
                <thead>
                  <tr className="bg-slate-50 dark:bg-zinc-800/50 text-slate-400 dark:text-zinc-500">
                    ...
                    <th className="text-left px-6 py-4 font-black uppercase tracking-widest text-[10px]">Institution</th>
                    <th className="text-left px-6 py-4 font-black uppercase tracking-widest text-[10px]">Board</th>
                    <th className="text-center px-6 py-4 font-black uppercase tracking-widest text-[10px]">Year</th>
                    <th className="text-right px-6 py-4 font-black uppercase tracking-widest text-[10px]">Score</th>
                  </tr>
                </thead>
                  ...
                  {educationDetails
                    ...
                    .map((edu, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition-colors">
                        <td className="px-6 py-5 font-black text-xs text-brand-primary uppercase">{edu.type}</td>
                        <td className="px-6 py-5 font-bold">{edu.school}</td>
                        <td className="px-6 py-5 text-slate-500">{edu.board}</td>
                        <td className="px-6 py-5 text-center font-bold">{edu.year}</td>
                        <td className="px-6 py-5 text-right">
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 dark:bg-zinc-800 font-black text-[11px] text-slate-700 dark:text-zinc-300">
                            {edu.percentage}% <span className="opacity-40 font-medium">{edu.division}</span>
                          </span>
                        </td>
                      </tr>
                    ))}
            ...
            <div className="space-y-4">
              {familyDetails
                ...
                .map((fam, idx) => (
                  ...
                  >
                    <div className="flex flex-col">
                      <Typography variant="body5" weight="black" className="text-rose-400 uppercase tracking-tighter text-[9px]">
                        {fam.relationLabel}
                      </Typography>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                 <DetailItem
                  ...
                  value={sourceOfInformation?.interviewedBefore === "yes" ? "Yes, Previously" : "No"}
                />
                <DetailItem
                  label="Past Employment"
                  value={sourceOfInformation?.workedBefore === "yes" ? "Yes, Previously" : "No"}
                />
              </div>
             
              <div className="pt-4 border-t border-slate-100 dark:border-zinc-800/50">
                <Typography variant="body5" weight="black" className="text-slate-400 uppercase tracking-widest text-[10px] mb-3 block">
                  Discovery Channels
                </Typography>
...
function SectionCard({ title, icon, children, accentColor, bgIcon: BgIcon }: { title: string; icon: React.ReactNode; children: React.ReactNode; accentColor: string; bgIcon?: React.ElementType }) {
  return (
    <Card className="h-full bg-white dark:bg-zinc-900/50 backdrop-blur-sm border border-slate-200 dark:border-zinc-800 p-8 rounded-[2rem] shadow-sm dark:shadow-[0_15px_40px_rgba(0,0,0,0.4)] dark:ring-1 dark:ring-zinc-800/50 hover:shadow-md dark:hover:shadow-[0_25px_60px_rgba(0,0,0,0.6)] transition-all duration-500 relative overflow-hidden group">
      <div className={`absolute top-0 left-0 w-1.5 h-full ${accentColor} opacity-20`} />
      ...
      <div className="flex items-center gap-4 mb-8 relative z-10">
        <div className={`h-10 w-10 rounded-xl ${accentColor}/10 flex items-center justify-center text-slate-600 dark:text-zinc-400`}>
          {icon}
        </div>
        <Typography variant="h4" weight="black" className="uppercase tracking-widest text-[13px] text-slate-800 dark:text-zinc-200">
          {title}
        </Typography>
      </div>
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <Typography variant="body3" weight="bold" className="text-slate-900 dark:text-zinc-100">
          {value || "—"}
        </Typography>
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 opacity-40">
      <div className="h-12 w-12 rounded-full border-2 border-dashed border-slate-300 mb-3 flex items-center justify-center">
        <Typography variant="body1" className="text-slate-300">?</Typography>
      </div>
      <Typography variant="body4" className="italic">{message}</Typography>
    </div>
  );
}
