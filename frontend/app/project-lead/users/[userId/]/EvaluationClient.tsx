...
export default function EvaluationClient({ userId }: EvaluationClientProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [resR1, resVerdicts] = await Promise.all([
          resultsApi.getUserResultDetail(userId),
          classificationsApi.getClassifications({ type: "interview_verdict", is_active: true }),
        ]);
        setR1Data(resR1);
        const rawVerdicts = resVerdicts?.data ?? resVerdicts ?? []; setVerdicts(Array.isArray(rawVerdicts) ? rawVerdicts : []);
      } catch (err) {
    ...
    <PageContainer className="py-6 space-y-8 max-w-7xl mx-auto pb-40">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          disabled={submitting}
          className="shadow-xl shadow-brand-primary/20 px-8 h-12 rounded-2xl font-bold"
