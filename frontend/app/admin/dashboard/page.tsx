...
export default function DashboardPage() {
  ...
  const { data: overview, isLoading: overviewLoading, isFetching: isRefetching } = useDashboardOverview({
  ...
  const pulseMetrics = [
    { label: "New Registrations", value: today_pulse?.registrations ?? 0, icon: <UserPlus />, sub: "Fresh applicants", color: "text-blue-500", bgColor: "bg-blue-500/10" },
    { label: "Re-interviews", value: today_pulse?.reinterviews ?? 0, icon: <RefreshCcw />, sub: "Candidates returning", color: "text-purple-500", bgColor: "bg-purple-500/10" },
    { label: "Paper Assignments", value: today_pulse?.assignments ?? 0, icon: <ClipboardCheck />, sub: "Auto & Manual allot", color: "text-emerald-500", bgColor: "bg-emerald-500/10" },
    { label: "Completed Tests", value: today_pulse?.attempts ?? 0, icon: <CheckCircle2 />, sub: "Finalized submissions", color: "text-amber-500", bgColor: "bg-amber-500/10" },
  ];
            ...
            <MainCard
              title={
                <div className="flex items-center gap-3">
                  ...
                  <Typography variant="h4" weight="black" className="uppercase tracking-widest pt-0.5 text-foreground">Dashboard Pulse</Typography>
                </div>
              }
              className="h-full"
            >
